...
// "c-reciente" espera menos que WATCHDOG_MIN_SILENCE_MS (produccion lo bajo de
// 15 min a 3 min el 2026-08): 1 minuto sigue siendo "recien escribio".
const CONVOS = [
  ...
  { id: "c-reciente", phone_number: "51944444444", kapso: { contact_name: "Mia", last_inbound_at: iso(60000), last_outbound_at: iso(50 * 60000), last_message_text: "y el precio?" } },
...
(async () => {
  ...
  // Una alerta por cada chat del equipo: produccion difunde a varios chat ids.
  assert.ok(telegramSent.length >= 1, "debe enviarse al menos una alerta");
  for (const msg of telegramSent) {
    assert.ok(msg.text.includes("Daph"), "la alerta debe incluir a Daph");
    assert.ok(msg.text.includes("o no vendes?"), "la alerta debe incluir el texto");
    assert.ok(!msg.text.includes("Luis"), "no debe alertar cierres triviales");
    assert.ok(!msg.text.includes("Mia"), "no debe alertar a quien acaba de escribir");
  }
