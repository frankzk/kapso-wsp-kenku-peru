...
function buildNote(input, customerLookup) {
  ...
  const alternatePhone = sanitizeNoteText(input.alternatePhone || input.alternate_phone);
  if (alternatePhone) lines.push(`Otro numero que dio el cliente: ${alternatePhone}`);
...
function enrichCustomerFromContext(input, payload) {
  ...
  const chatPhone = context.phone_number || contact.wa_id || wa.conversation?.phone_number;
  if (chatPhone && !input.whatsappPhone && !input.whatsapp_phone) {
    input.whatsappPhone = chatPhone;
  }

  if (!customer.phone && !input.phone && chatPhone) {
    input.phone = chatPhone;
  }
...
// El chat de WhatsApp ya expone el celular del cliente salvo en los leads que
// entran por username. Cuando lo expone, ESE numero es el contacto de entrega:
// si el agente manda cualquier otra cosa en phone (un fijo, digitos sueltos,
// "por coordinar"), no se pierde el pedido — se usa el del chat y lo que dio el
// cliente queda anotado en la orden. Sin numero en el chat, la puerta de
// phone_missing sigue igual: ahi el celular es imprescindible.
// Caso real: conversacion de Javier (31/08/2026), el bot le pidio el celular,
// el cliente respondio tres veces con el numero del propio chat y el bot le
// contesto que no era valido.
function applyContactPhoneFallback(input) {
  if (!input || typeof input !== "object") return;
  if (!input.customer || typeof input.customer !== "object") input.customer = {};
  const customer = input.customer;

  const given = String(customer.phone || input.phone || "").trim();
  if (isPeruMobile(given)) {
    const normalized = normalizePhone(given);
    customer.phone = normalized;
    input.phone = normalized;
    return;
  }

  const chatPhone = String(input.whatsappPhone || input.whatsapp_phone || "").trim();
  if (!isPeruMobile(chatPhone)) return; // no hay nada mejor: decide phone_missing

  const normalizedChat = normalizePhone(chatPhone);
  if (given && !samePhoneDigits(given, normalizedChat)) {
    input.alternatePhone = given;
  }
  customer.phone = normalizedChat;
  input.phone = normalizedChat;
}
...
globalThis.__kenkuCreateShopifyOrder = {
  applyContactPhoneFallback,
  buildNote,
  buildOrderInput,
  enrichCustomerFromContext,
  isPeruMobile,
  normalizePhone,
  computePricing,
  countFreeUnits,
